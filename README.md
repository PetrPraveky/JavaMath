...
- **Stieltjes generalized constants** ("*src/lib/math/complex/function/generalized_stieljes_constant.java*") - Generalization of stieljes constants. Its done with approx integral calculation and thanks to wikipedia: *https://en.wikipedia.org/wiki/Stieltjes_constants#Generalized_Stieltjes_constants*.
...
- **Beta function** ("*src/lib/java/math/plotter/complex_function_plotter/complex_beta_plotter.java*") - Plotted with domain coloring. There are two plots. Both are plotted from *-5* to *5* on both axis and. First plot of beta function *B(x,y)*, where *x = a+bi* and *y = b+ai* (*b*, *a* are areal numbers). <br />
