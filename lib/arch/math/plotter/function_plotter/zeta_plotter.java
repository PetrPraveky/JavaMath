...
public class zeta_plotter extends JPanel{
    ...
    protected void paintComponent(Graphics grf) {
        ...
        graph.drawString("Riemann zeta function", margin+20, margin); // Right corner legend
        // Set color of points
        graph.setPaint(Color.BLUE);
        // Draw points
        for (int i = 0; i < coords.size()-1; i++) {
            double x1 = -1.5*margin+coords.get(i).get(0)*x;
            double y1 = height/2-coords.get(i).get(1)*(amountOfParts*(10));
            double x2 = -1.5*margin+coords.get(i+1).get(0)*x;
    ...
    public zeta_plotter() {
        ...
        for (double i = min; i <= max; i += amountOfParts) {
            ...
            if (i == 1) {             
                ...
                coord.add(i*mul); 
                coord.add(zeta._riemann_zeta_function(i)*mul);
                coords.add(coord);
            }
        }
    }
