...
public class complex_trigamma_plotter extends JPanel {
    ...
    protected void paintComponent(Graphics grf) {
        ...
        graph.drawString("Trigamma function", margin+20, margin+12); // Right corner legend
    }
    public complex_trigamma_plotter() {
        complex_trigamma_function gamma = new complex_trigamma_function();
        ...
        for (double i = min; i <= max; i += amountOfParts) {
            for (double j = min; j <= max; j += amountOfParts) {
                ...
                if (Math.floor(i) == i && i <= 0 && j == 0) {
                    ...
                    BigComplexNumbers result = gamma._trigamma_function(new BigComplexNumbers(new BigDecimal(i), new BigDecimal(j)));
