...
public class complex_tanh_plotter extends JPanel {
    ...
    protected void paintComponent(Graphics grf) {
        ...
        graph.drawString("Hyperbolic tangent function", margin+20, margin+12); // Right corner legend
    }
        ...
        for (double i = min; i <= max; i += amountOfParts) {
            for (double j = min; j <= max; j += amountOfParts) {
                ...
                if (i == 1 && j == 0) {
                    ...
                    ComplexNumbers result = (new ComplexNumbers(i, j)).tanh();
